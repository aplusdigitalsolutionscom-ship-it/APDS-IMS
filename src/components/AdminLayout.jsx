...
const WarrantyCertificate = lazy(() => import('./WarrantyCertificate'));
...
export default function AdminLayout() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      // Always fetch fresh user data from the server so permission changes
      // made by an admin take effect immediately without requiring re-login.
      try {
        ...
        // Network error or 401 — keep localStorage data; auth errors will
        // eventually redirect to login via the axios 401 interceptor.
  ...
  return (
    <div className="flex h-screen bg-slate-50 relative">
      ...
      <aside className={`bg-white border-r flex flex-col hidden md:flex print:hidden transition-all duration-300 ease-in-out ${isSidebarVisible ? 'w-70 opacity-100' : 'w-0 opacity-0 overflow-hidden border-none'}`}>
        ...
        {['users','activity','reports','profile','settings','notifications'].includes(activeTab) ? (
          <>
        ...
        <nav className="flex-1 px-2 space-y-1 overflow-y-auto">
          ...
          {/* 8. SETTINGS – navigates to /profile (triggers settings mode) */}
          <button
