...
export default function CategoryBrandMapping() {
  ...
  const fetchDependencies = async () => {
    try {
      const [catRes, brandRes] = await Promise.all([
        axios.get(`${API_BASE_URL}/Inventory/GetCategoryList`, { headers: getHeaders() }),
        axios.get(`${API_BASE_URL}/Inventory/GetBrandList`, { headers: getHeaders() })
      ]);
  ...
  const fetchMappings = async () => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/Inventory/GetCategoryBrandMappingList`, {
        headers: getHeaders(),
      });
  ...
  const handleSaveMapping = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE_URL}/Inventory/SaveOrUpdateCategoryBrandMapping`,
        payload,
        { headers: getHeaders() }
      );
  ...
  const handleDeleteMapping = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setTableLoading(true);
        try {
          const res = await axios.post(`${API_BASE_URL}/Inventory/DeleteCategoryBrandMapping`, 
            { mappingId: id }, 
            { headers: getHeaders() }
          );
  ...
  return (
    <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 min-h-screen">
      <div className="flex items-center gap-4 border-b border-slate-100 pb-6 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">Category Brand Mapping</h2>
          <p className="text-sm text-slate-500 mt-1 font-medium">Link brands to specific categories</p>
        </div>
      </div>
