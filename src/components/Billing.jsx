...
export default function Billing({
    ...
    const billingDispatches = useMemo(() => {
        ...
        return dispatches.filter(d => {
            ...
            if (activeTab === "billing") {
                const isSendForBilling = d.status === "Send for Billing" || d.status === "Send for Billing (Hold)";
                const isHold = d.status === "Order On Hold" || d.status === "Order Not Confirmed";
                
                // Automatically show Hold orders if they are >= 7 days old
                let is7DaysOld = false;
                if (isHold) {
                    const refDateStr = d.dispatchDate || d.orderDate || d.createdAt;
                    const refDate = refDateStr ? new Date(refDateStr) : new Date();
                    is7DaysOld = differenceInDays(new Date(), refDate) >= 7;
                }

                return isSendForBilling || (isHold && is7DaysOld);
            } else {
    ...
    const dueBillsCount = useMemo(() => {
        ...
        return dispatches.filter(d => {
            if (!d || d.isDeleted || (d.status !== "Send for Billing" && d.status !== "Send for Billing (Hold)")) return false;
    ...
    const handleEditClick = (group) => {
        ...
        setBatchItems(group.map(item => ({ id: item.guid || item.id, serialGuid: item.serialGuid || item.serialNumberId })));
    ...
    const handleSaveEdit = async (e) => {
        ...
        // ✅ UPDATED: Validate for "Send for Packing" and "Save and Back"
        if ((editForm.status === "Send for Packing" || editForm.status === "Save and Back") && !editForm.invoiceNo.trim()) {
            alert("⚠️ Invoice Number is required.");
            return;
        }

        // ✅ NEW: E-Way Bill validation — mandatory if order > ₹50,000
        if ((editForm.status === "Send for Packing" || editForm.status === "Save and Back") && isEwayBillRequired) {
        ...
        if (editForm.status === "Send for Packing" || editForm.status === "Save and Back") {
            // "Save and Back" means they want to save invoice info but haven't sent for packing yet.
            // If it's a Hold order (or previously sent for billing hold), we keep it pending billing or put it to Billed.
            // User requested that once billed, it should go to dispatch like any other order.
            const isHold = editingBatch && editingBatch[0] && (
                editingBatch[0].status === "Send for Billing (Hold)" || 
                editingBatch[0].status === "Order On Hold" || 
                editingBatch[0].status === "Order Not Confirmed"
            );
            
            if (editForm.status === "Save and Back") {
                 finalStatus = isHold ? "Send for Billing (Hold)" : "Send for Billing";
                 finalLogisticsStatus = null;
            } else {
                 // Send for Packing -> Go to Dispatch!
                 finalStatus = "Billed";
                                        ...
                                        >
                                            ...
                                            <td className="p-4 text-center">
                                                <div className="flex items-center gap-2 justify-center">
                                                    ...
                                                    {activeTab === 'billing' ? (
                                                        <button disabled={!canManage} onClick={() => handleEditClick(group)} title="Generate Bill" className="flex items-center gap-1 px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition shadow-sm text-xs font-bold w-full justify-center disabled:opacity-50 disabled:cursor-not-allowed">
                                                            <Edit2 size={12} />
                                                            Process
                                                        </button>
                                                    ) : (
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 my-auto">
                        ...
                        <div className="p-6 max-h-[75vh] overflow-y-auto">
                            {(() => {
                                ...
                                const isECommerce = ["Amazon", "Flipkart"].includes(details.firmName);
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm font-medium text-slate-700 flex items-center gap-1">
                                                        <Calendar size={12} />
                                                        {details.invoiceDate ? format(new Date(details.invoiceDate), "dd MMM yyyy") : "-"}
                                                    </p>
                                                </div>
                                                <div>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 animate-in zoom-in-95 my-auto">
                        ...
                        <form onSubmit={handleSaveEdit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1"><UploadCloud size={12} /> {editingBatchIsMarketplace ? "Original Invoice (From Order)" : "Upload Invoice"}</label>
                                        <input
                                            type="file"
                                        ...
                                        />
                                        {editForm.existingInvoice && (
                                            <div className="mt-2 flex items-center justify-between bg-indigo-50 px-3 py-2 rounded-lg border border-indigo-100">
                                                <div className="flex items-center gap-2">
                                                    <FileText size={12} className="text-indigo-600" />
                                                    <a 
                                                        href={`import.meta.env.VITE_API_URL/uploads/${editForm.existingInvoice}`} 
                                                        target="_blank" 
                                                        rel="noopener noreferrer"
                                                        className="text-[11px] text-indigo-800 font-bold hover:underline truncate max-w-[150px]"
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => setEditForm(prev => ({ ...prev, existingInvoice: "", removeInvoice: true }))}
                                                    className="p-1 text-red-500 hover:text-red-700 hover:bg-red-100 rounded-md transition"
                                ...
                                <div className={`p-4 rounded-xl border transition-all ${isEwayBillRequired
                                    ...
                                    <div className="flex items-center justify-between mb-3">
                                        <label className={`text-xs font-bold uppercase flex items-center gap-1.5 ${isEwayBillRequired ? "text-red-600" : "text-slate-500"}`}>
                                            <FileText size={14} />
                                            {editingBatchIsMarketplace ? "Accountant Upload (Optional Doc/Challan)" : "E-Way Bill Upload"}
                                            {isEwayBillRequired ? (
                                    ...
                                    {editForm.existingEwayBillName && !editForm.ewayBillFile && (
                                        <div className="mt-3 flex items-center justify-between bg-emerald-50 px-3 py-2.5 rounded-lg border border-emerald-200">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                    <p className="text-xs text-emerald-800 font-bold">{editingBatchIsMarketplace ? "Current Document" : "Current E-Way Bill"}</p>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <a
                                                    href={`import.meta.env.VITE_API_URL/uploads/${editForm.existingEwayBillName}`}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => setEditForm(prev => ({ ...prev, existingEwayBillName: "", removeEwayBill: true }))}
                            ...
                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                ...
                                >
                                    <option value={editingBatch && editingBatch[0] && editingBatch[0].status === "Send for Billing (Hold)" ? "Send for Billing (Hold)" : "Send for Billing"}>Keep Pending</option>
                                    <option value={editingBatch && editingBatch[0] && editingBatch[0].status === "Send for Billing (Hold)" ? "Save and Back" : "Send for Packing"}>
                                        {editingBatch && editingBatch[0] && editingBatch[0].status === "Send for Billing (Hold)" ? "Save and Back" : "Send for Packing"}
                                    </option>
                                </select>

                                {/* ✅ UPDATED: Status hints */}
                                {(editForm.status === "Send for Packing" || editForm.status === "Save and Back") && (
                                    <div className="mt-2 p-2.5 bg-cyan-50 border border-cyan-200 rounded-lg flex items-start gap-2">
                                        ...
                                        <div>
                                            <p className="text-[11px] text-cyan-800 font-bold">
                                                {editForm.status === "Save and Back" 
                                                    ? "✅ Order details will be saved and sent back." 
                                                    : "✅ Order will be sent to Packing stage"}
                                            </p>
                                            <p className="text-[10px] text-cyan-600 mt-0.5">
                                                {editForm.status === "Save and Back" 
                                                    ? "Status will be updated to Billed (Hold)"
                                ...
                                {String(editForm.status).startsWith("Send for Billing") && (
                                ...
                                {(editForm.status === "Send for Packing" || editForm.status === "Save and Back") && isEwayBillRequired && !editForm.ewayBillFile && !editForm.existingEwayBillName && (
                            ...
                            <div className="flex gap-3 justify-end pt-2">
                                ...
                                <button type="submit" className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition flex items-center gap-2">
                                    {(editForm.status === "Send for Packing" || editForm.status === "Save and Back") ? (
                                        <>
                                            <Package size={14} /> {editForm.status === "Save and Back" ? "Save & Back" : "Save & Send for Packing"}
                                        </>
                                    ) : (
