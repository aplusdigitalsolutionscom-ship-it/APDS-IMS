...
const modalTypes = {
  WAREHOUSE_SELECT: 'warehouse_select',
  ADD: 'add',
  SELL: 'sell',
  DETAILS: 'details',
  EDIT: 'edit'
...
export default function FbfFbaManagement({ isAdmin, isUser, currentUser }) {
  ...
  const handleSellStock = async (event) => {
    ...
    setSaving(true);
    try {
      const result = await printerService.sellFbfFbaStock({
...
function CategoryChoice({ icon: Icon, title, description, active, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function SummaryTile({ icon: Icon, label, value, tone = 'indigo' }) {
  const tones = {
...
function Modal({ title, onClose, children, size = 'md', closeOnEscape = true }) {
  useEffect(() => {
