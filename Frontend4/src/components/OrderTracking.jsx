...
function isHoldStatus(status) {
  return status === "Order On Hold" || status === "Order Not Confirmed" || status === "Send for Billing (Hold)" || status === "Billed (Hold)";
}
...
export default function OrderTracking({
  ...
  const handleSendBackToBilling = async (batch) => {
    setBillingLoadingId(batch.batchKey);
    try {
  ...
  const handleSendToDispatch = async (batch) => {
    setBillingLoadingId(batch.batchKey);
    try {
      const status = "Billed";
                      ...
                      <div className="space-y-4">
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <div>
                              <label className="text-[10px] font-semibold text-slate-600 block mb-1">Invoice Date</label>
                              <input type="date" className="w-full border border-slate-200 bg-white p-2 rounded-lg text-xs focus:ring-2 focus:ring-amber-500 outline-none"
                      ...
                      <>
                                      ...
                                      <td className="px-3 py-2.5 text-center">
                                        {isReplacing ? (
                                          ...
                                          <div className="flex items-center justify-center gap-2">
                                            ...
                                            {(isAdmin || isSupervisor) && (
                                              <button
                                                onClick={() => handleRemoveSerial(item.guid || item.id, itemSerial)}
                        ...
                        {(() => {
                          ...
                          const EXTRA_DOC_OPTIONS = [
                            "Inspection Certificate", "Acceptance Certificate", "CAMC Agreement",
                            "Purchase Order Copy", "Delivery Challan", "Warranty Certificate",
                            "Test / QC Report", "Performance Bank Guarantee", "Installation Report", "Other",
                          ];
                              ...
                              <div className="p-3 space-y-3">
                                  ...
                                  <div className="space-y-1.5">
                                    {extraDocs.map((doc, idx) => (
                                      <div key={idx} className="flex items-center justify-between bg-violet-50 border border-violet-100 rounded-lg px-3 py-2">
                                        ...
                                        <button onClick={() => handleViewDocument(doc.filename)} className="ml-2 flex-shrink-0 flex items-center gap-1 text-[11px] font-bold text-violet-700 hover:text-violet-900 bg-violet-100 hover:bg-violet-200 px-2.5 py-1 rounded-lg transition-colors">
                                          <Eye size={11} /> View
                                        </button>
                                      </div>
                                    ))}
                                ...
                                {canEditOrder && !isCancelledOrder && (
                                  <div className="border border-dashed border-violet-300 bg-violet-50/50 rounded-xl p-3 space-y-2.5">
                                    ...
                                    >
                                      {EXTRA_DOC_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                    </select>
                                    {extraDocType === "Other" && (
                                      <input
                                        type="text"
                                        placeholder="Document ka naam likho (e.g. CAMC Letter)"
                                      ...
                                      <label className="flex-1 cursor-pointer">
                                        <div className={`flex items-center gap-2 border rounded-lg px-3 py-2 transition-colors ${extraDocFile ? "border-violet-400 bg-violet-100" : "border-violet-200 bg-white hover:bg-violet-50"}`}>
                                          ...
                                          <span className={`text-xs font-medium truncate ${extraDocFile ? "text-violet-700" : "text-slate-400"}`}>
                                            {extraDocFile ? extraDocFile.name : "File choose karo (PDF, DOC, JPG, PNG)"}
                                          </span>
                                        </div>
                                        <input
                                    ...
                                    <button
                                      ...
                                      disabled={!extraDocFile || uploadingExtraDoc || (extraDocType === "Other" && !extraDocCustomLabel.trim())}
                                ...
                                {extraDocs.length === 0 && !canEditOrder && (
                                  <p className="text-xs text-slate-400 italic text-center py-3">Koi additional document upload nahi hua hai.</p>
                                )}
                              </div>
                            </div>
                          );
