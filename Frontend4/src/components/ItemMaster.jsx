...
export default function ItemMaster() {
  ...
  const fetchDependencies = async () => {
    try {
      const [catRes, unitRes] = await Promise.all([
        ...
        axios.get(`${API_BASE_URL}/Inventory/GetUnitList`, { headers: getHeaders() })
      ]);
  ...
  const fetchBrandsForCategory = async (catId) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/Inventory/GetBrandByCategory?categoryId=${catId}`, {
        headers: getHeaders(),
      });
  ...
  const handleSaveItem = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE_URL}/Inventory/SaveOrUpdateItem`,
        payload,
        { headers: getHeaders() }
      );
  ...
  const handleDeleteItem = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setTableLoading(true);
        try {
          const res = await axios.post(`${API_BASE_URL}/Inventory/DeleteItem`, 
            { itemId: id }, 
            { headers: getHeaders() }
          );
  ...
  return (
    <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100 min-h-screen">
      <div className="flex items-center gap-4 border-b border-slate-100 pb-6 mb-8">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">Item Master</h2>
          <p className="text-sm text-slate-500 mt-1 font-medium">Manage base items for your inventory</p>
        </div>
      </div>
