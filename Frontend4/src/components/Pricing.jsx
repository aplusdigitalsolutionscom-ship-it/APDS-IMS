...
export default function Pricing({ models = [], onRefresh, isAdmin }) {
  ...
  const handleModelChange = (modelGuid) => {
    const latestEntry = pricingHistory.find(
      p => p.modelGuid?.guid === modelGuid
    );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.modelGuid || !form.price || !form.purchaseDate) {
    ...
    try {
      ...
      setForm({ modelGuid: "", price: "", purchaseDate: today });
      loadPricings();
  ...
  const handleDelete = async (id) => {
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "Delete this price record? This action cannot be undone.",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire({
        title: "Deleted!",
        text: "The price record has been deleted successfully.",
        icon: "success",
          ...
          <tbody className="divide-y">
            {pricingHistory.map(p => (
              <tr key={p.id} className="hover:bg-slate-50">
                <td className="p-4 font-medium text-slate-700">{p.modelGuid?.name || "Deleted Model"}</td>
