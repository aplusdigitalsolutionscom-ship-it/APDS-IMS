...
import { 
  ...
  ArrowLeft, Package, PackageOpen, List, Info, Edit, Loader2, Type
} from 'lucide-react';
...
const ComboMaster = () => {
  ...
  const fetchAllVariants = async () => {
    try {
       ...
       const res = await axios.get(`${API_BASE_URL}/Inventory/GetCurrentStock`, { params: { limit: 1000 } });
  ...
  const fetchComboList = async (page = currentPage, limit = pageSize) => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/Inventory/GetComboList`, {
        params: { page, limit },
          ...
          <div className="bg-slate-50 border-t border-slate-200 px-6 py-4 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <select 
                  className="bg-white border border-slate-200 rounded-lg px-2 py-1 text-sm font-bold text-slate-600 outline-none focus:border-fuchsia-400 transition-all cursor-pointer"
                  value={pageSize}
                  onChange={(e) => {
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="p-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-white hover:text-fuchsia-600 disabled:opacity-30 disabled:hover:bg-transparent transition-all"
              >
                   ...
                   return (
                    <button
                      ...
                      className={`w-10 h-10 rounded-lg text-sm font-bold transition-all ${
                        currentPage === pageNum ? 'bg-fuchsia-600 text-white shadow-md shadow-fuchsia-100' : 'text-slate-600 hover:bg-white hover:text-fuchsia-600'
                      }`}
                    >
                      {pageNum}
                    </button>
